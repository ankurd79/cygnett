//alert('hello');
						...
						$("#frmmedia_add").validate({
			                ...
			                messages: {
			                    news_title: {
			                        required: "Specify News title",
			                        minlength: jQuery.validator.format("At least {0} characters required!"),
			                        //maxlength: jQuery.validator.format("A maximum of 30 characters allowed.")
			                    },

			                    url: {
			                        required: "Specify external url",
			                        minlength: jQuery.validator.format("At least {0} characters required!"),
			                        //maxlength: jQuery.validator.format("A maximum of 30 characters allowed.")
			                    },

			                    source: {
			                        required: "Specify news source",
			                        minlength: jQuery.validator.format("At least {0} characters required!"),
			...
			$("#frmmedia_add").on('submit',(function(e) {
				//alert('hello');
			        e.preventDefault();
			        if($("#frmmedia_add").valid()){
			            $.ajax({
			                type:'POST',
			                cache: false,
			                contentType: false,
			                processData: false,
			                url:base_url+'add-media-news',
			                ...
			                //alert(data);
								    ...
								    if(data=='Media News added.'){
										...
										$(function() {	
										    ...
										    Toast.fire({
										        icon: 'success',
										        title: ' Media News added.'
	      									})
	      								});
										
									}else if(data=='Media News already present!'){
									    
									    //jQuery("#redalert").show();
									    //jQuery("#redalert").html(data);
									    $('html, body').animate({scrollTop: '0px'}, 300);
									    $("#news_title").css("background-color", "#FA8072");
									    $(function() {	
										    ...
										    Toast.fire({
										        icon: 'error',
										        title: ' Media News already present!'
	      									})
	      								});
									}
			...
			$("#frmmedia_edit").on('submit',(function(e) {
										//alert('hello');
									        e.preventDefault();
									        if($("#frmmedia_edit").valid()){
									            $.ajax({
									                type:'POST',
									                cache: false,
									                contentType: false,
									                processData: false,
									                url:base_url+'update-media-news',
									                data:new FormData(this),
									                beforeSend: function ( xhr ) {
									                    //Add your image loader here
									                     jQuery(".lds-facebook").show();
									                ...
									                //alert(data);
														    ...
														    Toast.fire({
														        icon: 'success',
														        title: 'Media news information updated.'
					      									})
	      											});
			...
			$("#news_title,#url,#source").change(function(){

            ...
            $finalArray=array('hotel_name'=>$data['hotel_name'],'tag_line'=>$data['tag_line'],
              ...
              foreach($nearby as $key=>$value){
                if(strlen($nearby[$key])>0){
                  $nearbyInsert=array('hotel_id'=>$lastinsertedId,'nearby_area'=>$value,'nearby_detail'=>$data['nearby_detail'][$key],'added_on'=> date_stamp(),'added_by' => $user,'is_active'=>1);
...
public function updateData($data,$user){
                ...
                foreach($nearby_area as $key=>$value){
                    if(strlen($nearby_area[$key])>0){
                      ...
                      if(isset($data['nearby_id'][$key])){
                          $arrnearbyUpdate=array('nearby_area'=>$value,'nearby_detail'=>$data['nearby_detail'][$key],'updated_on'=> date_stamp(),'updated_by' => $user);
                          $this->db->where('id',$data['nearby_id'][$key]);
                          $this->db->update(hotel_nearby,$arrnearbyUpdate);
                      }else{
                          $arrnearbyInsert=array('hotel_id'=>$updateid,'nearby_area'=>$value,'nearby_detail'=>$data['nearby_detail'][$key],'added_on'=> date_stamp(),'added_by' => $user,'is_active'=>1);
    ...
    public function getData($id='',$site='',$coming=''){
      $this->db->select('u.*,s.name, l.location, c.nicename'); 
            ...
            #$lastinsertedId=1;
              ...
              foreach($text as $key=>$value){
                  if(strlen($text[$key])>0){
                    ...
                    $arrInsert=array('image'=>$uploadedfile,'hotel_id'=>$unit_id,'text'=>$text[$key],'is_active'=>1,'added_on'=> date_stamp(),'added_by' => $user);
              ...
              foreach($text as $key=>$value){
                  if(strlen($text[$key])>0){
                    ...
                    if($image[$key]){
                      ...
                      $arrupdate=array('image'=>$uploadedfile,'text'=>$text[$key],'updated_on'=> date_stamp(),'updated_by' => $user);

                    }else{
                      $arrupdate=array('text'=>$text[$key],'updated_on'=> date_stamp(),'updated_by' => $user);
      ...
      public function updateunitbanners($data,$user){
              ...
              foreach($bannerimage as $key=>$value){
                                ...
                                if(isset($data['banner_id'][$key])){
                                  ...
                                  $upArray=array('position'=>$data['position'][$key],'updated_on'=> date_stamp(),'updated_by' => $user,'is_active'=>1);
                                  ...
                                  if(strlen($bannerimage[$key])>0){   
                                      ...
                                      if(strlen($bannerimage[$key])>0){
                                        $finalArray=array('banner_image'=>$uploadedfile,'hotel_id'=>$unit_id,'page_category_id'=>$page_category_id[$key],'added_on'=> date_stamp(),'added_by' => $user,'is_active'=>1);
